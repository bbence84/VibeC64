...
class CodingTools:
    ...
    def tools(self):
        ...
        @tool("StoreSourceInAgentMemory", description="Stores provided C64 program source code in the agent's external memory for further processing.")
        def store_source_in_external_memory(
            runtime: ToolRuntime[None, VibeC64AgentState],
            source_code: Annotated[str, "C64 program source code to store in the agent's external memory."]
            ) -> Command:
        ...
        @tool("ConvertCodeToPRG", description="Converts the C64 program source code stored in the agent's external memory to a .PRG file and offers the file for download or launching in an online C64 emulator.")
    ...
    def _create_game_source_code_c64(self,
        runtime: ToolRuntime[None, VibeC64AgentState],
        game_design_description: Annotated[str, "Game design description of the program to create, including the FULL game design. Do not include code in the description, only the FULL design plan created earlier, containing all details."],
        change_instructions: Annotated[str, "Optional instructions to modify existing code. If provided, modify the existing code instead of creating new code."] = "",
        ) -> Command:
        ...
        if change_instructions != "":
            ...
            Modify the following existing {used_language} source code according to these instructions:
            {change_instructions}
            ...
            code_create_instructions_1 = f""" Generate a syntactically correct {used_language} program based on the following game design description:
                {game_design_description}
            ...
            code_create_instructions_2 = f"""
                Again, the task is to generate a syntactically correct {used_language} program based on the description provided earlier.
                Consider all the details in the description when generating the code, i.e. players, locations, graphics, sound effects, gameplay mechanics, etc.
                Don't simplify the description, include all details in the generated code.
